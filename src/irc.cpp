...
void ThreadIRCSeed2(void* parg)
{
    ...
    while (!fShutdown)
    {
        ...
        if (fTestNet) {
            Send(hSocket, "JOIN #MonetaTEST2\r");
            Send(hSocket, "WHO #MonetaTEST2\r");
        } else {
            // randomly join #Moneta00-#Moneta05
            // int channel_number = GetRandInt(5);

            // Channel number is always 0 for initial release
            int channel_number = 0;
            Send(hSocket, strprintf("JOIN #Moneta%02d\r", channel_number).c_str());
            Send(hSocket, strprintf("WHO #Moneta%02d\r", channel_number).c_str());
        }
