...
Alternatively, install Qt Creator and open the `Moneta-qt.pro` file.

An executable named `Moneta-qt` will be built.
...
A warning for people using the *static binary* version of Moneta on a Linux/UNIX-ish system (tl;dr: **Berkely DB databases are not forward compatible**).

The static binary version of Moneta is linked against libdb5.3.
...
installed causes Moneta-qt to crash intermittently.  The issue has been reported as `launchpad bug 857790`_, but
isn't yet fixed.
